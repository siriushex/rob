...
namespace Invector.vCharacterController
{
    ...
    public class vThirdPersonController : vThirdPersonAnimator, ISpawnableEntity
    {
        /// <summary>
        ...
        {
            base.OnDisable();
            GameController.OnSpawnAll -= SpawnToRandomPoint;
            GameController.OnReturnAll -= ReturnToStartPoint;
        }
...
{
    Debug.Log("OnEnable vThirdPersonController");
    GameController.OnSpawnAll += SpawnToRandomPoint;
    GameController.OnReturnAll += ReturnToStartPoint;
    
    // Добавляем проверку на null и оборачиваем в try-catch
    try
    {
        if (GameController.Instance != null) 
        {
            GameController.Instance.RigistredPlayer();
            playerId = 0;
            Debug.Log("Игрок успешно зарегистрирован");
        }
        else
        {
            Debug.LogWarning("GameController.Instance еще не инициализирован!");
            // Добавляем отложенную регистрацию через StartCoroutine
            StartCoroutine(RegisterPlayerWhenReady());
        }
    }
    catch (System.Exception e)
    {
        Debug.LogError("Ошибка при регистрации игрока: " + e.Message);
    }
}

// Корутина для отложенной регистрации
private IEnumerator RegisterPlayerWhenReady()
{
    // Ждем, пока GameController будет инициализирован
    yield return new WaitUntil(() => GameController.Instance != null);
    
    // Регистрируем игрока
    GameController.Instance.RigistredPlayer();
    playerId = 0;
    Debug.Log("Игрок успешно зарегистрирован (отложенная регистрация)");
}

    
        public void SpawnToRandomPoint(Transform[] points)
        {
            Debug.Log("SpawnToRandomPoint");
            Transform point = points[Random.Range(0, points.Length)];
            transform.position = point.position;
            transform.rotation = point.rotation;
        }

        public void ReturnToStartPoint(Transform[] points)
        {
            Transform point = points[Random.Range(0, points.Length)];
            transform.position = point.position;
            transform.rotation = point.rotation;
